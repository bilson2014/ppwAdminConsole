	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					/* 控件功能拓展---》老王 */
					fromData:[],				  // 修改控件实现提交文件可以携带参数传递到后台。fromData 以Key/Value形式传递
					/* 提供给外部的接口方法 */
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<img class="upload_image" src="/resources/lib/zyUpload/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "/resources/lib/zyUpload/images/fileType/";
