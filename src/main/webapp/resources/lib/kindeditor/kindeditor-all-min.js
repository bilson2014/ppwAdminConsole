...
_extend(KUploadButton, {
	init : function(options) {
		...
		var html = [
			...
			'<input type="hidden" name="storage_" value="'+getDfsHostName()+'" />',
...
KEditor.prototype = {
	...
	html : function(val) {
		...
		//保存值，去掉host
	      if(self.isCreated){
	    	  var a = self.edit.html();
	    	  //不靠谱的正则，测试服务器http://123.123.1.1满足 正式http://123.123.1.1/resource 不满足
	    	  //var re2='((?:http|https)(?::\\/{2}[\\w]+).*?([/|/resource]))';	// HTTP URL 1
	    	  var re2='(src="http)([^ ]*)(/group)';	// HTTP URL 1
		      var p = new RegExp(re2,["gm"]);
		     // var x2=a.replace(p, "@.@");
		      var x2=a.replace(p, 'src="@.@group');
		      return x2;
	      }else{
	    	  return _elementVal(self.srcElement);
	      }
	      //modify end
